...
type Bucket interface {
	Get(ctx context.Context, key string) (uint, time.Time, error)
	Set(ctx context.Context, key string, count uint, startTime time.Time) error
...
func (rl *RateLimitter) Check(ctx context.Context, key string) (bool, error) {
	count, startTime, err := rl.bucket.Get(ctx, key)
	if err != nil {
	...
	if time.Since(startTime) > rl.interval {
		return true, rl.bucket.Set(ctx, key, 1, time.Now())
	}
	...
	return true, rl.bucket.Set(ctx, key, count+1, startTime)
}

func (rl *RateLimitter) Remove(ctx context.Context, key string) error {
	return rl.bucket.Remove(ctx, key)
}
